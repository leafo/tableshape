...
-- lua source of clone_state, inlined into standalone generated modules.
-- Must match the behavior of clone_state above
CLONE_STATE_SOURCE = [[local clone_state = function(state_obj)
...
class Compiler
  ...
  -- assemble a standalone lua module with no dependencies. FailedTransform
  -- is minted locally, so the sentinel never crosses the module boundary:
  -- the exported functions convert it to the public nil, error convention
  assemble_module: (main_name, error_message) =>
    assert #@refs == 0, "cannot assemble a standalone module with runtime references"
    ...
    err_expr = ("%q")\format error_message

    footer = {
      "local check_value = function(value, state)"
      "local v, s = #{main_name}(value, state)"
      "if v == FailedTransform then return nil, #{err_expr} end"
...
-- module has no dependencies (not even tableshape) and returns a table with
-- check_value & transform functions matching the BaseType interface. The
-- type must compile statically (see generate_code), and the output is
-- deterministic: the same type generates byte identical source
generate_module = (node) ->
  assert BaseType\is_base_type(node), "expected type checker to compile"
  ...
  compiler\assemble_module main_name, error_message
...
{ :compile, :generate_code, :generate_module, :CompiledType, :Compiler }
