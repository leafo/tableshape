...
if c1[rk] == nil then
local item = value[rk]
local tuple, s = t2({[rk] = item}, state)
if tuple == FailedTransform then return FailedTransform end
state = s
local nk = tuple and next(tuple)
if nk == nil then
dirty = true
else
if nk ~= rk or tuple[nk] ~= item then dirty = true end
out[nk] = tuple[nk]
end
end
end
...
if v == FailedTransform then return nil, "expected {  }" end
